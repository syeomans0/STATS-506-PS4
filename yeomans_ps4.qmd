...
year_2014 <- nzge %>% 
            ...
            ungroup()
as.data.frame(year_2014) #converted to a data frame bc tibble only gave first 10
```
...
who_won_what
```
For this part, I found which party won the Candidate vote and which party won the Party vote for each election year. I first grouped the data by election year, voting type, and party, then summed the total number of votes each party got After that, I determined which party had the highest total votes within each year and voting type, which gave the winning party. The reason why total votes and max votes are equal is since i used the filter where this is the case.
...
head(play_win2)
```
We do see that some players won multiple tournaments. In total 171 players won more than one tournament and the player with the most amount of wins was Rafael Nadal. This seems like a lot of players winning, but it comes from the fact there are multiple matches within the tournament itself and we aren't told an ``overall'' winner of the tournament in the data set. 

Once again, I first attempted this without realizing there were the three tournaments still in 2018. Going back and excluding this one date, we still find some players won multiple tournaments and in total had 170 players who won more than one tournament match. The player with the most amount of wins is the same, Rafael Nadal with 60 wins.
...
head(win_rate)
```
Lots of the players with the highest win rate showed up in the previous part with the players with the highest wins. The player with the highest win rate is Rafael Nadal with a win rate of $86.95%$, they had 60 wins and 9 losses. The player with the second highest win rate is Novak Djokovic with a rate of $84.05%$. Went back in and added the filter excluding 2018.

I used a full join on the data set because I wanted to match the IDs of the winners and losers to a name, and the full join returns in a final data set, all rows, and all columns from both data sets. I found how to combine two tidyverse tibbles and how to do a full join using <https://datascienceplus.com/merging-datasets-with-tidyverse/>. 
...
```
There were 2 major spikes and 5 minor spikes in monthly cases. The major spikes are in December, 2020 and January, 2022. The smaller, minor, spikes are April, 2020, July, 2020, August, 2021, July, 2022, and December 2022. I do know spikes are subjective so people might count more or less than me.
...
ggplot(data = nyt_highlow, aes(x = date, y= cases_avg_per_100k, color = state)) +
    ...
    theme_bw()
```
For this problem, I started by investigating which state had the highest and lowest of the max cases average per 100k for their individual state. After this, we found the state with the highest cases average per 100k and the one with the lowest which were American Samoa and Maine, respectively. There were lots of states that had zero, but every state has at least some nonzero values eventually. Since this is the case we look for the highest and lowest of the maximum values of cases average per 100k, allowing me to compare the extremes in the outbreak. 

The graph shows their trajectories over the course of the pandemic. Focusing on the blue line, Maine, we see that they experiences COVID starting in mid-2020 and had a peak at the start of 2021. After this, their number of average cases per 100k goes down until we hit the middle of 2021 where we see the trend go up pretty consistently. It drops again around, best guess, March 2022. After that we see one more small spike but then the number of average cases gradually goes down. Switching our focus to the red line, American Samoa, we see a different story. COVID didn't reach American Samoa until late 2021 and it hits hard, as we see a huge spike at the beginning of 2022, reaching over 500 average cases per 100k. After this huge spike, it drops right back down but we see two more spikes around July 2022 and September 2022 (guess at months based on the years on the bottom). 
...
Therefore the first five states to to experience COVID in a substantial way are NY, NJ, LA, MA, and CT. This is very close to the actual answer I found earlier. Below is a graph of these 5 states for the whole year of 2020. I feel like this makes sense intuitively since the states with bigger populations and diversity most likely experiences COVID first, but I was kind of surprised by LA, since it is in the South and not as populated as the other states in the first 5.  
...
gg_5_4 <- ggplot(top_five, aes(x = date, y = cases_avg_per_100k)) +
          geom_line() +
          labs(title = "First 5 States to Experience COVID in a Substantial Way (2020)",
                   x = "Date",
                   y = "Cases on Average per 100K") +
          facet_wrap(~ state) +
          theme_bw()
